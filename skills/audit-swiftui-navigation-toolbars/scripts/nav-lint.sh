#!/usr/bin/env bash
# nav-lint.sh — thin pointer. The toolkit ships ONE shared hybrid lint engine fed declarative rule files
# (the pattern every audit skill inherits); there is no bespoke per-skill bash script.
#
