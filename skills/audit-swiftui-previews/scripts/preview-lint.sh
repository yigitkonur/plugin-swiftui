#!/usr/bin/env bash
# preview-lint.sh — thin pointer. The toolkit ships ONE shared hybrid lint engine fed declarative rule
# files (the pattern every audit skill inherits); there is no bespoke per-skill grep script.
#
