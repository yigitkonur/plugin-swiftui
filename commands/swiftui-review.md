...
!`"${CLAUDE_PLUGIN_ROOT}/bin/swiftui-ctx" deprecated 2>/dev/null | head -20 || echo "swiftui-ctx unavailable"`

Now:
1. Read the target file(s) (or `git diff` if no path given).
2. For each SwiftUI symbol used, check `"${CLAUDE_PLUGIN_ROOT}/bin/swiftui-ctx" deprecated <api>` (flag + replacement)
