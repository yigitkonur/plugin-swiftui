...
The runner: `<swiftui-plugin-root>/scripts/swiftui-lint.sh`.
...
A skill's lint rules live in `<swiftui-plugin-root>/skills/<skill-name>/lint/`:
...
3. In `SKILL.md`, the LOCATE step + "Detection accelerator" line call
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill <skill> --dir <sources> --json … --sarif …`,
