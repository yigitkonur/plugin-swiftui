...
`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`. Floors for the *real* replacements
...
3. **Verify before flagging a near-miss.** If a name *looks* invented but might be a real new
   symbol, confirm via Sosumi (`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`)
