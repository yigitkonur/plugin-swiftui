...
- `availability` is read from
  `<swiftui-plugin-root>/references/_shared/floors-master.md` — never asserted from memory.
- `source` cites Apple primary docs (fetched per
  `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`) or carries the
...
- `cross_ref` targets and primary-owner verdicts derive from
  `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` — the same source the orchestrator's
...
Apple-doc fetch path all live in sibling `_shared/` files referenced above via `<swiftui-plugin-root>`.
