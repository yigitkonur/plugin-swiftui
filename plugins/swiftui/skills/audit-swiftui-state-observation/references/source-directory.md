...
`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`; the Apple-doc fetch protocol (curl/CLI
commands + JSON-404 caveat) is `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`. Floor values
are `<swiftui-plugin-root>/references/_shared/floors-master.md` (read, never restate).
...
1. **Practice — how do shipping Mac apps write this?** Run
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` → read `consensus` (the canonical
...
- swiftui-ctx CLI contract: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` (the
