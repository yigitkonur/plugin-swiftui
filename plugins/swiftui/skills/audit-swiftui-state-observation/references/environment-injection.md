...
```bash
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Environment --json        # @Environment(Type.self) consensus
bash <swiftui-plugin-root>/scripts/swiftui-ctx recipe observable-model          # the own-once-inject-by-type pattern
bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart    # the real `.environment(_:)` site, live
```
