...
```bash
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup ObservationIgnored --json  # introduced_macos 14.0, real call sites
bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart     # a real @ObservationIgnored model, live
```
