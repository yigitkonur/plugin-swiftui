...
```bash
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup State --json        # @State re-instantiation idiom; recommended example
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Observable --json   # recommended ex_44cfa1bff8 = @Observable @MainActor
bash <swiftui-plugin-root>/scripts/swiftui-ctx file ex_44cfa1bff8 --smart # the @MainActor model, live from GitHub
```
