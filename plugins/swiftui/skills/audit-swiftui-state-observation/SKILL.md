...
description: Audit macOS SwiftUI state observation for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Observable --json` then
...
are the reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` (read, never
restate); the canonical invented-name list is
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`. Signatures, the per-wrapper
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-state-observation --dir <sources> --json /tmp/state.json --sarif /tmp/state.sarif`.
...
5. **VERIFY.** For anything ≤ ~70% confidence (a wrapper floor you can't place, a behavior claim, a
   ...
   - **Practice — swiftui-ctx:** `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json`
     (read `consensus`, `recommended`, `co_occurs_with`, `introduced_macos`, `deprecated`) and, for any
     "is this deprecated" doubt, `bash <swiftui-plugin-root>/scripts/swiftui-ctx deprecated <api>`. A
   ...
   - **Spec — Sosumi:** `curl -sSL https://sosumi.ai/<apple-path>` for floor/signature, via
     ...
     `developer.apple.com`).
   Cross-check the floor against `<swiftui-plugin-root>/references/_shared/floors-master.md`. Promote
...
| `<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` | the canonical invented-name list (no state symbol is on it) |
| `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` | the macOS-arm gating rule for the macOS-14 / macOS-26 floors |
| `<swiftui-plugin-root>/references/_shared/finding-schema.md` | the unified finding schema + the `model_kind`/`failure_shape` additive fields |
| `<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md` | the 8-point fix-safety protocol (step 7) |
| `<swiftui-plugin-root>/references/_shared/sosumi-reference.md` | the Apple-doc fetch protocol (step 5 VERIFY, spec side) |
| `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` | the swiftui-ctx CLI contract (step 5 VERIFY practice side + step 7 FIX consensus/permalink) |
| `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` | seam ownership + `cross_ref` targets (view-performance, concurrency-safety, swiftdata, async-data, previews) |

## Detection accelerator

`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-state-observation --dir <files-or-dir>
...
+ JSON/SARIF shape + safety rails: `<swiftui-plugin-root>/references/_shared/lint-architecture.md`.
