...
`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`; this file is the pointer/gesture-specific
*map* of which pages to fetch. Floor values live in
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the practice layer (consensus + permalinks)
is `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
...
2. **Is it deprecated in practice?** `bash <swiftui-plugin-root>/scripts/swiftui-ctx deprecated <api>` →
   `deprecated`/`migrate_to` (pg-08/09). A `lookup` **exit 3** corroborates pg-01.
3. **What's the canonical shape?** `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` →
...
- Sosumi fetch protocol + JSON-404 caveat: `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`.
- swiftui-ctx CLI contract: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
- All Apple paths above fetched via `https://sosumi.ai/...` (access 2026-06-07); floors cross-checked
  against `<swiftui-plugin-root>/references/_shared/floors-master.md`.
