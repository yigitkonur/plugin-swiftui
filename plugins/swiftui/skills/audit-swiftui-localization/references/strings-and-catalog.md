...
`<swiftui-plugin-root>/references/_shared/floors-master.md`. The ✅ shapes below are the **swiftui-ctx
consensus** for each API (run `swiftui-ctx lookup <api> --json` to refresh the permalinked example);
verify any uncertain symbol via Sosumi (`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`).
