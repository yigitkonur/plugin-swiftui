...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; this file carries the trap mechanics, the
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup MenuBarExtra --json` →
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx recipe menubar-app` pattern pairs `MenuBarExtra` +
