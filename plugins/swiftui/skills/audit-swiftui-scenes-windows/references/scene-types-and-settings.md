...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the invented-name list lives in
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`. This file carries the type
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx deprecated WindowGroup --json`.
...
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`; the scene-specific ❌→✅ below.
...
`Button("Cancel")`. The consensus settings shape is `bash <swiftui-plugin-root>/scripts/swiftui-ctx
...
All Apple-doc availability strings cross-checked against `<swiftui-plugin-root>/references/_shared/floors-master.md`
