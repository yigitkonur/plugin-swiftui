...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; this file carries the mechanics, the
...
`Window`. The consensus open shape is `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup openWindow --json`.
...
`<swiftui-plugin-root>/references/_shared/cross-ref-graph.md`). The consensus is
`bash <swiftui-plugin-root>/scripts/swiftui-ctx recipe window-scene` (real examples: `jordanbaird/Ice`,
...
Apple availability strings cross-checked against `<swiftui-plugin-root>/references/_shared/floors-master.md`
