...
commands and the JSON-404 caveat is `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`; this
file is the sandbox/files-specific *map* of which pages to fetch. Floor values live in
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the practice corpus (consensus shape +
permalinked example) is reached with `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json`
per `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
...
- The practice corpus contract: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
