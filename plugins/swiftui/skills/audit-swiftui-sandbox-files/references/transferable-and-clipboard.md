...
example; fetch it with `bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart`.
...
`<swiftui-plugin-root>/references/_shared/floors-master.md`, the successor floors at macOS 26.0. Note the
...
Floors are cross-checked against `<swiftui-plugin-root>/references/_shared/floors-master.md`
...
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`; pages fetched via Sosumi (access
