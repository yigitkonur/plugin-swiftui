...
guard newer trait/label forms. Floors are in `<swiftui-plugin-root>/references/_shared/floors-master.md`.
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx file ex_e9a36e4789 --smart` →
...
per `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`. The blanket "is every floored API gated"
