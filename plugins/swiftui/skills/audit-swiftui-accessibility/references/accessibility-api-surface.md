...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the canonical invented-name list is
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` — read, never restate. For the
**canonical call shape** of any modifier below, run `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup
