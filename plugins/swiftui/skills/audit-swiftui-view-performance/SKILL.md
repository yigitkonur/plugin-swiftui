...
description: Audit macOS SwiftUI view performance for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` — read, never restate.
...
live via `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Table --json` → consensus
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-view-performance --dir <sources> --json /tmp/vperf.json --sarif /tmp/vperf.sarif`.
...
5. **VERIFY.** For anything ≤ ~70% confidence (a fix target whose floor you can't place, a behavior
   ...
   **Spec** — confirm via **Sosumi**: `curl -sSL https://sosumi.ai/<apple-path>` using
   ...
   is `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`. Carry the `Table` threshold
...
7. **FIX.** Apply corrections under the fix-safety protocol
   ...
   **not a hand-written snippet** — it is the swiftui-ctx **consensus shape** put in `## Correct`,
   ...
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart` whose GitHub
...
| Shared file | For |
|---|---|
| `<swiftui-plugin-root>/references/_shared/floors-master.md` | every floor/availability value (the reconciled truth — e.g. `Text(_:format:)`: macOS 12.0+ (String) / 15.0+ (AttributedString)) |
...
| `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` | seam ownership + `cross_ref` targets (state-observation, animation-motion, drawing-canvas, layout-and-tables, appkit-interop, liquid-glass) |

## Detection accelerator

`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-view-performance --dir <files-or-dir>
