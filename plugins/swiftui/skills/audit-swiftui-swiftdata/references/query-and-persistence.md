...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the macOS-arm gating rule lives in
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`. Get the canonical `@Query` ✅ from
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Query --json` returns the real idiom: the
