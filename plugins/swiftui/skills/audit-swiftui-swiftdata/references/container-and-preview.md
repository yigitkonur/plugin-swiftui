...
`<swiftui-plugin-root>/references/_shared/floors-master.md`. Get the canonical container ✅ from
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup ModelContainer --json` returns the dominant
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart`; put its permalink + the
