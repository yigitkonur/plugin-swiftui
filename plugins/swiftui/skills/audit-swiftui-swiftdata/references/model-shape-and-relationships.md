...
`<swiftui-plugin-root>/references/_shared/floors-master.md`. Get the canonical ✅ shape from
...
```
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Relationship --json   # consensus shape
bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Model --json          # @Model usage + co_occurs_with
bash <swiftui-plugin-root>/scripts/swiftui-ctx file <recommended.id> --smart   # the real enclosing body
```
