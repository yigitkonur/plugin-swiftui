...
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` — read it, do not restate it here.
...
equivalent — never to gate it. Per `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` §3,
...
use the JSON availability endpoint per `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`,
...
- The macOS-ABSENT / invented-name list + Mac equivalents:
  `<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` and the reading-the-string rule
  in `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` §3 (toolkit-internal, Apple-sourced
  via Sosumi, access 2026-06-07).
- Floor values + the type-property quirk: `<swiftui-plugin-root>/references/_shared/floors-master.md`.
