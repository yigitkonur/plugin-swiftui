...
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`. Floor *values* are the reconciled truth
in `<swiftui-plugin-root>/references/_shared/floors-master.md` — that table **is** this skill's floor
...
double-report (per `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md`). If it is a glass
...
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` §2 this is a gating finding, **not** a
...
- The gating rule, the `*` wildcard, the wrong-arm failure, multi-platform strings:
  `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` (toolkit-internal, Apple-sourced via
  Sosumi, access 2026-06-07).
- Floor values: `<swiftui-plugin-root>/references/_shared/floors-master.md` (same provenance).
