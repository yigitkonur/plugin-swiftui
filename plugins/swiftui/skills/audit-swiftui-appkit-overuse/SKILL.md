...
description: Audit macOS SwiftUI appkit overuse for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-appkit-overuse --dir <sources> --json /tmp/over.json --sarif /tmp/over.sarif`.
...
5. **VERIFY.** For any ≤ ~70%-confidence call (does the SwiftUI replacement exist? at this floor? does it
   ...
   **Deeper corpus evidence (WHETHER-to-bridge):** when a bridge looks like overuse, prove real apps don't
   need it — `bash <swiftui-plugin-root>/scripts/swiftui-ctx bridges <kind-or-name> --json` (stable envelope +
...
7. **FIX.** **Flag-only** under the fix-safety protocol
   (`<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md`) — never auto-delete a
...
| `<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md` | the fix-safety protocol (step 7) — this domain is flag-only |
...
`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-appkit-overuse --dir <files-or-dir>
