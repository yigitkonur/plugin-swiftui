...
VERIFY run `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <SwiftUI-api> --json` for the
...
`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`. Floor values:
...
Per `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md`: **overuse owns WHETHER**, interop owns
