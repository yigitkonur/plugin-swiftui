...
description: Audit macOS SwiftUI navigation toolbars for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` and the canonical
platform-wrong-name list in `<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` §5 —
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-navigation-toolbars --dir <sources> --json /tmp/nav.json --sarif /tmp/nav.sarif`.
...
| `<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md` | §5 toolbar/navigation platform-wrong placements |
| `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` | the macOS-arm gating rule + the `macOS ABSENT` "replace, never gate" rule (nav-05, nav-11) |
...
`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-navigation-toolbars --dir <files-or-dir>
