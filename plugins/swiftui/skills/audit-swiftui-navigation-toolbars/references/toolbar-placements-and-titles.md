...
values are the reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` — never
...
Per `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`: a `macOS ABSENT` symbol is **replaced,
...
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`). If the project floor is already ≥ 26,
...
`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`); access 2026-06-07. Floors live in
