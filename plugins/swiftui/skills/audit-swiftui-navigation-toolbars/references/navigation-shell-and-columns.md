...
`<swiftui-plugin-root>/references/_shared/floors-master.md` — never restated here.** Every ✅ shape is
...
All Apple docs fetched via Sosumi (protocol: `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`);
access 2026-06-07. Floors live in `floors-master.md`; the live consensus shape + permalink come from
`swiftui-ctx lookup NavigationSplitView` (see `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`).
