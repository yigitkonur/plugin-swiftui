...
`<swiftui-plugin-root>/references/_shared/floors-master.md` — read them there; the dates below are the
era labels for the `era` finding field, not a restated floor table.** The canonical ✅ shape is the
`swiftui-ctx` consensus, not the snippet below: `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup
...
net). The wrong-arm rule is `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`.
