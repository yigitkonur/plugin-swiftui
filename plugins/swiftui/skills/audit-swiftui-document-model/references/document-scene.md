...
`<swiftui-plugin-root>/references/_shared/floors-master.md`; the macOS-arm rule is
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`. Verify via Sosumi + `swiftui-ctx`.
