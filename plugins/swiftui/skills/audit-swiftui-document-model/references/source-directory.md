...
https://sosumi.ai/<apple-path>`; protocol in `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`
— never `WebFetch` `developer.apple.com`). Paired with the practice corpus via `swiftui-ctx lookup <api>
--json` (`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`). Use this to resolve a
