...
lives in `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`, and floor *values* are the
reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md`. This file adds which
...
the wrong-arm failure in `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` — flag it, gate
...
- Apple — fetched via Sosumi (access 2026-06-07): `/documentation/swiftui/scene/commandsremoved()`
  ...
  in `source-directory.md` + `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`.
- Floor values: `<swiftui-plugin-root>/references/_shared/floors-master.md`. Gating rule:
...
- Practice (floor cross-check): `swiftui-ctx lookup <api> --json` `introduced_macos`
  (`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`), accessed 2026-06-07.
