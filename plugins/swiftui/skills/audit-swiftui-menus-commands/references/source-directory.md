...
`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`; this file is the menus-specific *map* of
which pages to fetch. The PRACTICE half (consensus shape + permalinked example) is `swiftui-ctx`
(`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`). Floor values live in
...
- Practice layer: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` (`swiftui-ctx
