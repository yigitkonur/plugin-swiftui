...
`<swiftui-plugin-root>/references/_shared/floors-master.md` — read, never restate. The ✅ here is the
...
Per `<swiftui-plugin-root>/references/_shared/floors-master.md` it is **`macOS ABSENT`** (a compile error
on non-Catalyst macOS), **not** an under-gated symbol. Do **not** wrap it in `#available(macOS …)` — that is
the "macOS ABSENT is not a low floor" trap in `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`.
