...
the orchestrator's dedup) is `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` — the
...
- `## Correct` body = **(a)** a one-line route — *"Run `audit-swiftui-pointer-gestures` on this file to
  ...
  hand-written snippet: `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` → put its
...
- **Never apply a code change here.** No `fix_mode: auto`; the fix-safety protocol
  (`<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md`) is invoked only for its hand-off
...
ownership + valid slugs live in `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md`; the
`swiftui-ctx` CLI contract is `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
