...
description: Audit macOS SwiftUI macos nativeness for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
`<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` (the `macos-nativeness` row) — read it,
never restate it.
...
reconciled values live in `<swiftui-plugin-root>/references/_shared/floors-master.md`; never restated
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-macos-nativeness --dir <sources> --json /tmp/nat.json --sarif /tmp/nat.sarif`.
...
5. **VERIFY.** For any affordance whose existence/shape/floor you are < ~100% sure of, run **both**
   evidence sources. (a) **Practice** — `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api>
   ...
   **exit 3** means the symbol you expected is not real. (b) **Spec** — confirm the floor via **Sosumi**:
   ...
   `<swiftui-plugin-root>/references/_shared/sosumi-reference.md` (never `WebFetch` developer.apple.com).
   ...
   **Deeper corpus evidence (benchmark the score):** anchor the 0-100 to the real-Mac-app baseline —
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx stats --json` (`.result.modern_stack`) +
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx insights modern-stack --json`
   (`.result.data` = `modern_stack_adoption_pct`) +
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx rankings most_modern_stack --json`. Over 1,857 shipping
...
7. **ROUTE (this skill's "FIX").** This skill is `fix_mode: flag-only` for **every** smell — it applies
   **no** code change. The `## Correct` of each finding is **not a fix here**; it is (a) a one-line "run
   ...
   (`<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md`) still governs: since nothing is
...
| `<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md` | the fix-safety protocol (step 7 — here only the no-auto-fix / hand-off clause) |
| `<swiftui-plugin-root>/references/_shared/sosumi-reference.md` | the Apple-doc spec fetch protocol (step 5 VERIFY) |
| `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` | the practice-corpus CLI contract — `lookup`/`deprecated`/`file --smart` for the consensus affordance shape + permalink (steps 5 VERIFY · 7 ROUTE) |
...
`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-macos-nativeness --dir <files-or-dir>
