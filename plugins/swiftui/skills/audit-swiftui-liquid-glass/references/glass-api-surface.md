...
are not restated here — they live in `<swiftui-plugin-root>/references/_shared/floors-master.md` (the
...
Confirm any floor against `<swiftui-plugin-root>/references/_shared/floors-master.md`; verify any
uncertain symbol via Sosumi (`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`).
...
`<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`; the glass-specific ❌→✅ rewrites
