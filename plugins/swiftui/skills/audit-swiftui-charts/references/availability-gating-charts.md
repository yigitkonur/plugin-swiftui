...
*values* are the reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` — read,
never restate them here. This file is the **gating logic** for the Charts domain. The macOS-arm rule and
the wrong-arm failure mode are in `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md`.
