...
commands + the JSON-404 caveat) is `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`; this
file is the drawing-specific *map* of which pages to fetch. Floor values live in
`<swiftui-plugin-root>/references/_shared/floors-master.md`. The **practice** half of VERIFY is
`swiftui-ctx` (`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`).
...
2. **Practice cross-check.** `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` →
...
- swiftui-ctx practice CLI: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` (run
