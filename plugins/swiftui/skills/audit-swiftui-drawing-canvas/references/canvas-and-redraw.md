...
`<swiftui-plugin-root>/references/_shared/floors-master.md`. The ✅ "Correct" shapes below are the
**swiftui-ctx consensus** for each API (get the live one with `bash <swiftui-plugin-root>/scripts/swiftui-ctx
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup Canvas --json` then `file <recommended.id> --smart`.
...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup TimelineView --json`. Warning, `flag-only` — the
...
- Practice corpus (consensus shapes + permalinked call sites): `swiftui-ctx lookup Canvas|TimelineView|
  ...
  (accessed 2026-06-07). CLI contract: `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
