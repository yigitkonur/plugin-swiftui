...
`<swiftui-plugin-root>/references/_shared/sosumi-reference.md`. Pair every spec check with the **practice**
read: `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` (+ `recipe nsview-bridge`,
`deprecated <api>`). Floors reconcile against `<swiftui-plugin-root>/references/_shared/floors-master.md`.
