...
description: Audit macOS SwiftUI appkit interop for correctness, current APIs, and production conventions. Use for that domain or as part of a full audit.
---
...
rather than a hand-written snippet. `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup
...
2. **LOCATE.** Run the shared hybrid lint runner:
   `bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-appkit-interop --dir <sources> --json /tmp/interop.json --sarif /tmp/interop.sarif`.
...
5. **VERIFY.** For anything ≤ ~70% confidence (a member you're unsure is required, a floor you can't
   ...
   **Deeper corpus evidence (this domain):** when you doubt a bridge shape is real or want the make/update/
   ...
   `bash <swiftui-plugin-root>/scripts/swiftui-ctx bridges NSViewControllerRepresentable` (stable envelope +
...
7. **FIX.** This domain is `fix_mode: flag-only` end-to-end (every fix needs human bridge context) — leave
   ...
   (`<swiftui-plugin-root>/references/_shared/fix-safety-protocol.md`: clean-tree gate, never weaken a
...
| `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` | the macOS-arm gating rule + wrong-arm failure (interop-10 gates) |
| `<swiftui-plugin-root>/references/_shared/finding-schema.md` | the unified finding schema + frontmatter keys (incl. `swift_era`/`isolation_kind`) |
...
| `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` | the practice-corpus CLI contract — `lookup`/`recipe`/`file --smart` for the consensus shape + permalinked example (steps 5 VERIFY · 7 FIX) |
| `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` | seam ownership + `cross_ref` targets (appkit-overuse · concurrency-safety · appearance-color) |

## Detection accelerator

`bash <swiftui-plugin-root>/scripts/swiftui-lint.sh --skill audit-swiftui-appkit-interop --dir <files-or-dir>
