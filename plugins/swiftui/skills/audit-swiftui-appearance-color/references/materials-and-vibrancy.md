...
Liquid Glass lies. Floor values: `<swiftui-plugin-root>/references/_shared/floors-master.md` (read, never
restate). Get the ✅ shape from the corpus: `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup
<api> --json`.
...
Seam authority: `<swiftui-plugin-root>/references/_shared/cross-ref-graph.md` (appearance-color ↔
