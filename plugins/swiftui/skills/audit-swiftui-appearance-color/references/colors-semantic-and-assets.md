...
reconciled truth in `<swiftui-plugin-root>/references/_shared/floors-master.md` (read, never restate); the
invented-name list is `<swiftui-plugin-root>/references/_shared/hallucination-blacklist.md`. Get the ✅
shape from the corpus, not memory: `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json`.
