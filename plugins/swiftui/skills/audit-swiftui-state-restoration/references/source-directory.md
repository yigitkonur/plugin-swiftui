...
curl/CLI commands and the JSON-404 caveat is `<swiftui-plugin-root>/references/_shared/sosumi-reference.md`;
this file is the domain-specific *map* of which pages to fetch. Floor values live in
`<swiftui-plugin-root>/references/_shared/floors-master.md`. The practice corpus is the swiftui-ctx CLI
...
3. **Practice cross-check.** `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` returns
