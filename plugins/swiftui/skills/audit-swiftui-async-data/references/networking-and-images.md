...
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup AsyncImage --json`,
...
spelling (see `<swiftui-plugin-root>/skills/build-macos-swiftui/references/concurrency.md` mistakes 1, 3, 6):
```swift
.task {
    let (data, _) = try await URLSession.shared.data(from: url)
