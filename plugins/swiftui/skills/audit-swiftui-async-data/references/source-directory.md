...
`<swiftui-plugin-root>/references/_shared/floors-master.md`. **Practice** (the consensus shape + a real
macOS-26 example) comes from `swiftui-ctx` per `<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md`.
...
2. **Need the consensus shape / a real example?** `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup
