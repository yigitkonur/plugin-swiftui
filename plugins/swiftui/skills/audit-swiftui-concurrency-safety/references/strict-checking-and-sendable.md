...
> `bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` and take the `consensus` shape +
> the `recommended` permalink rather than pasting a snippet (the FIX step requires the permalinked
> example). Floor values: `<swiftui-plugin-root>/references/_shared/floors-master.md`.
