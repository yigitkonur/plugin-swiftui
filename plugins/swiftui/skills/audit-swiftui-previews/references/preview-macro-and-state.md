...
from memory. Floor values: `<swiftui-plugin-root>/references/_shared/floors-master.md`.
...
`<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` §3); replace it with the plain macro.
...
`<swiftui-plugin-root>/references/_shared/swiftui-ctx-reference.md` and `sosumi-reference.md`). The ✅ in a
