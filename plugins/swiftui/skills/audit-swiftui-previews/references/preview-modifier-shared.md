...
- `PreviewModifier` / `.modifier(_:)` trait — **macOS 15.0+** (carried `verify-SDK` in
  `<swiftui-plugin-root>/references/_shared/floors-master.md`). Confirm via `swiftui-ctx lookup
...
- `ModelContainer.init(for:configurations:)` (variadic) is macOS 15.0+; `ModelConfiguration` /
  ...
  `<swiftui-plugin-root>/references/_shared/macos-arm-gating.md` if the floor is below 15.
