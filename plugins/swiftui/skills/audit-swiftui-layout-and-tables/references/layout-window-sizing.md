...
dev's call). Floors live in `<swiftui-plugin-root>/references/_shared/floors-master.md` — read, never
restate. The canonical shape for any API is what shipping Mac apps write — fetch it with
`bash <swiftui-plugin-root>/scripts/swiftui-ctx lookup <api> --json` (VERIFY) and back the ✅ with a
